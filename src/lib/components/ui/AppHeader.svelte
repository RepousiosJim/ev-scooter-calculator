<script lang="ts">
  ...
  const isWizard = $derived($page.url.pathname.startsWith('/wizard'));
  ...
  $effect(() => {
    // Track reactive dependencies so the effect re-runs when these change
    void uiState.activeTab;
    void isRankings;
  ...
  async function selectTab(value: string, _index: number) {
  ...
  async function handleKeydown(event: KeyboardEvent, index: number) {
    ...
    switch (event.key) {
      case 'ArrowRight':
        targetIndex = (index + 1) % calcTabs.length;
        break;
      case 'ArrowLeft':
        targetIndex = (index - 1 + calcTabs.length) % calcTabs.length;
        break;
      case 'Home':
        targetIndex = 0;
        break;
      case 'End':
        targetIndex = calcTabs.length - 1;
        break;
      default:
        return;
    }
  ...
  <div class="max-w-7xl mx-auto px-3 sm:px-4">
    <div class="h-14 lg:h-16 flex items-center justify-between gap-4">
      ...
      <nav class="hidden lg:flex items-center gap-1" aria-label="Main navigation">
        ...
        <div
          class="relative flex items-center bg-white/[0.04] border border-white/[0.06] rounded-full p-1"
          role="tablist"
          aria-label="Calculator tabs"
        >
          <!-- Sliding indicator -->
          <div
            class="absolute top-1 bottom-1 bg-primary/15 border border-primary/25 rounded-full transition-all duration-300 ease-out pointer-events-none"
          ...
          {#each calcTabs as tab, index (tab.value)}
            {#if isRankings}
              <a
                href="/"
                onclick={() => {
                  uiState.activeTab = tab.value as typeof uiState.activeTab;
                }}
                class="relative z-10 flex items-center gap-2 px-4 py-2 rounded-full text-xs font-bold uppercase tracking-[0.1em] text-text-tertiary hover:text-text-secondary transition-colors duration-200"
              >
            ...
            {:else}
              <button
                ...
                class="relative z-10 flex items-center gap-2 px-4 py-2 rounded-full text-xs font-bold uppercase tracking-[0.1em] transition-colors duration-200
                  focus:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-inset
                  {uiState.activeTab === tab.value ? 'text-primary' : 'text-text-tertiary hover:text-text-secondary'}"
              >
                <Icon
                  name={tab.icon}
                  size="sm"
                  class={uiState.activeTab === tab.value ? 'text-primary' : 'opacity-50'}
                />
                {tab.label}
              </button>
            {/if}
        ...
        <a
          href="/wizard"
          class="flex items-center gap-2 px-4 py-2 rounded-full text-xs font-bold uppercase tracking-[0.1em] transition-all duration-200
            {isWizard
        ...
        >
          <Icon name="search" size="sm" class={isWizard ? 'text-primary' : 'opacity-50'} />
          Finder
        </a>
        ...
        <a
          ...
          class="flex items-center gap-2 px-4 py-2 rounded-full text-xs font-bold uppercase tracking-[0.1em] transition-all duration-200
            ...
            : 'text-text-tertiary hover:text-text-secondary border border-transparent hover:border-white/[0.06] hover:bg-white/[0.03]'}"
        >
          <Icon name="book" size="sm" class={isGuides ? 'text-primary' : 'opacity-50'} />
          Guides
        </a>
      </nav>
      ...
      <nav class="lg:hidden flex items-center shrink-0">
        <a
          ...
          class="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-[0.12em] transition-all border
            {isRankings
