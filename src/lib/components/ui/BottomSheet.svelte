...
{#if isOpen}
  ...
  >
    ...
    <div
      ...
      class="w-full {heightMap[
        height
      ]} bg-bg-primary shadow-2xl rounded-t-2xl border-t border-x border-white/[0.06] overflow-hidden flex flex-col lg:max-w-2xl lg:rounded-xl lg:mx-4 relative"
    ...
    >
      <button
        ...
        type="button"
        onclick={onClose}
        class="absolute top-3 right-3 w-8 h-8 flex items-center justify-center rounded-full text-text-tertiary hover:text-text-primary hover:bg-white/10 transition-colors z-10"
        aria-label="Close preset selector"
      >
        ✕
      </button>
      {#if showHandle}
        <div class="flex justify-center pt-3 pb-2 cursor-grab active:cursor-grabbing" data-bottom-sheet-handle>
