...
export function generateRecommendations(config: ScooterConfig, stats: PerformanceStats): Recommendation[] {
  ...
  if (stats.speed < 60 && config.v < 72) {
    const voltageConfig: ScooterConfig = { ...config, v: Math.round(config.v * 1.2), watts: Math.round(config.watts * 1.2) };
  ...
  if (config.rollingResistance === undefined || config.rollingResistance >= 0.015) {
    ...
    const rangeGain = ((tireSpec.totalRange / currentSpec.totalRange - 1) * 100).toFixed(0);
