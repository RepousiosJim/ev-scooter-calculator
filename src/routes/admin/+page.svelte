	...
	const filteredScooters = $derived.by(() => {
		...
		result.sort((a: any, b: any) => {
			...
			switch (sortBy) {
				...
				case 'year': cmp = a.year - b.year || a.name.localeCompare(b.name); break;
				case 'confidence': cmp = a.overallConfidence - b.overallConfidence; break;
				case 'completeness': cmp = a.completeness - b.completeness; break;
				case 'sources': cmp = a.sourceCount - b.sourceCount; break;
			}
...
<div class="space-y-6">
		...
		<div class="bg-[#12121a] border border-gray-800 rounded-xl p-4 space-y-3">
			...
			<div class="space-y-2">
				...
				<div class="flex items-center justify-between text-xs">
					...
					<span class="text-white font-mono">{data.systemHealth.totalKnownSourceUrls} across {data.systemHealth.scootersWithSources} scooters</span>
				</div>
				<div class="flex items-center justify-between text-xs">
					...
					{#each data.recentActivity as entry}
						<div class="flex items-start gap-2 text-xs">
							<span class="w-1.5 h-1.5 rounded-full flex-shrink-0 mt-1
								{entry.type.includes('completed') || entry.type === 'login' || entry.type === 'seed_completed' ? 'bg-green-500' :
								 entry.type.includes('failed') ? 'bg-red-500' :
								 entry.type.includes('started') ? 'bg-cyan-500' : 'bg-gray-500'}"></span>
				...
				<div class="flex items-center gap-3">
					...
					<div class="flex items-center gap-1.5">
						{#if data.alerts.critical > 0}
							<span class="px-1.5 py-0.5 rounded-full text-[10px] font-bold bg-red-500 text-white">{data.alerts.critical} critical</span>
						{/if}
						{#if data.alerts.warning > 0}
							<span class="px-1.5 py-0.5 rounded-full text-[10px] font-bold bg-yellow-500/20 text-yellow-400">{data.alerts.warning} warnings</span>
						{/if}
					</div>
				</div>
				<a href="/admin/alerts" class="text-[10px] text-cyan-400 hover:text-cyan-300">View all {data.alerts.total} alerts &rarr;</a>
			</div>
			<div class="space-y-1.5">
				{#each data.alerts.topPriority as alert}
					<div class="flex items-center gap-2 text-xs px-2 py-1.5 rounded-lg bg-white/[0.02] hover:bg-white/[0.04] transition-colors">
						<span class="w-2 h-2 rounded-full flex-shrink-0
							{alert.severity === 'critical' ? 'bg-red-500' :
							 alert.severity === 'warning' ? 'bg-yellow-500' : 'bg-blue-500'}"></span>
						<span class="{alert.severity === 'critical' ? 'text-red-400' :
							alert.severity === 'warning' ? 'text-yellow-400' : 'text-blue-400'} font-medium flex-shrink-0">
							{alert.title}
						</span>
						<span class="text-gray-500 truncate">{alert.scooterName}</span>
						<a href={alert.actionUrl} class="ml-auto text-[10px] text-cyan-400 hover:text-cyan-300 flex-shrink-0">{alert.actionLabel || 'View'}</a>
					</div>
				{/each}
			</div>
		</div>
	...
	<div class="bg-[#12121a] border border-gray-800 rounded-xl p-4">
		...
		{#if importResult}
			<div class="mb-3 px-3 py-2 rounded-lg text-xs {importResult.includes('failed') ? 'bg-red-500/10 text-red-400' : 'bg-green-500/10 text-green-400'}">
				{importResult}
			</div>
		{/if}

		{#if seedResult}
			<div class="mb-3 px-3 py-2 rounded-lg text-xs {seedResult.error ? 'bg-red-500/10 text-red-400' : 'bg-green-500/10 text-green-400'}">
		...
		{:else if batchProgress}
			<div class="mt-2 px-3 py-2 rounded-lg text-xs {batchResult ? 'bg-green-500/10 text-green-400' : 'bg-red-500/10 text-red-400'}">
				{batchProgress}
			</div>
		{/if}
		...
		<div class="px-4 py-3 border-b border-gray-800 space-y-2">
			...
			<div class="flex flex-wrap items-center gap-2">
				...
				<button
					...
					class="px-2 py-1.5 text-xs text-gray-400 hover:text-white bg-[#0a0a0f] border border-gray-700 rounded-lg"
						...
						<tr class="border-b border-gray-800/50 hover:bg-white/[0.02] transition-colors">
							...
							<td class="px-4 py-3 text-center">
								{#if scooter.disputedCount > 0}
									<span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-medium bg-orange-500/10 text-orange-400">
										{scooter.disputedCount} disputed
									</span>
								{:else if scooter.verifiedCount > 0}
									<span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-medium bg-green-500/10 text-green-400">
										{scooter.verifiedCount} verified
									</span>
								{:else if scooter.sourceCount > 0}
									<span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-medium bg-blue-500/10 text-blue-400">
										has data
									</span>
								{:else}
