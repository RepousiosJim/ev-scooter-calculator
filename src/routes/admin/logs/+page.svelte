<script lang="ts">
	...
	const filteredEntries = $derived(
		filter === 'all' ? data.entries : data.entries.filter((e: any) => e.type === filter)
	);

	const filterTypes = $derived(() => {
		const types = new Set(data.entries.map((e: any) => e.type));
	...
	<div class="flex items-start justify-between">
		<div>
			...
			<p class="text-sm text-gray-400 mt-0.5">
				Track all admin actions, scans, and system events.
			</p>
		</div>
		<button
			onclick={clearLog}
					...
					<div class="flex-1 min-w-0">
						...
						{#if entry.details}
							<details class="mt-1">
								...
								<pre class="text-[10px] text-gray-500 mt-1 overflow-x-auto whitespace-pre-wrap">{JSON.stringify(entry.details, null, 2)}</pre>
							</details>
						{/if}
					</div>
				</div>
