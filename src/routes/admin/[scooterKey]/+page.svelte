<script lang="ts">
	...
	let autoVerifyResult = $state<any>(null);
	...
	function getFieldVerification(field: SpecField) {
		const fields = data.verification.fields as Record<string, any>;
		return (
			fields[field] || {
	...
	async function updateStatus(field: SpecField, status: VerificationStatus) {
		...
		const verifiedValue =
			status === 'verified' && fieldData.sources.length > 0
				? fieldData.sources[0].value
				: undefined;
...
<div class="space-y-6">
	...
	<div class="flex items-start justify-between">
		...
		<a
			href="/admin"
			class="text-xs text-gray-400 hover:text-white px-3 py-1.5 bg-white/5 rounded-lg transition-colors"
		>
			&larr; Back
		</a>
	</div>
			...
			<div class="mt-3 space-y-2">
				...
				<div class="w-full h-2 bg-gray-800 rounded-full overflow-hidden">
					<div
						...
						style="width: {autoVerifyProgress.total > 0 ? (autoVerifyProgress.completed / autoVerifyProgress.total) * 100 : 0}%"
					></div>
				</div>
					...
					<div class="max-h-40 overflow-y-auto bg-[#0a0a0f] border border-gray-800 rounded-lg p-2 space-y-0.5">
						{#each autoVerifyLog as entry}
							<div class="flex items-center gap-2 text-xs py-0.5 px-1">
								<span class="w-1.5 h-1.5 rounded-full flex-shrink-0 {entry.status === 'ok' ? 'bg-green-500' : 'bg-red-500'}"></span>
				...
				{:else}
					<div class="bg-green-500/5 border border-green-500/20 rounded-lg p-3 text-xs">
						<p class="text-green-400 font-medium mb-2">
							Scraped {autoVerifyResult.totalSources} sources: {autoVerifyResult.succeeded} succeeded, {autoVerifyResult.failed} failed
						</p>
						...
						<div class="ml-4 space-y-1.5 mb-2">
							{#each fieldVerification.sources as source}
								<div class="flex items-center justify-between text-xs bg-white/[0.02] rounded-lg px-3 py-2">
									<div class="flex items-center gap-2">
										<span class="px-1.5 py-0.5 rounded text-[10px] font-medium {sourceTypeColors[source.type] || 'bg-gray-500/10 text-gray-400'}">
											{source.type}
										</span>
										<span class="text-gray-300">{source.name}</span>
										<span class="text-white font-mono font-medium">
											{source.value} {spec.unit}
										</span>
										{#if source.url}
											<a href={source.url} target="_blank" rel="noopener noreferrer" class="text-cyan-400 hover:text-cyan-300">
												link
											</a>
										{/if}
									</div>
